...
class VideoEncoder(nn.Module):  ## CNN编码器
    ...
    def forward(self, video):
        ...
        video_input = video.transpose(1, 2)  # B C seq_len H W
        token_output = self.Conv3D(video_input).transpose(1,2)
        token_output = self.fc(token_output) # B seq_len C
        return  token_output



class VAEModel(nn.Module): ## Transformer Encoder
    def __init__(self,
                 in_channels: int,
                 latent_dim: int = 256,
                 **kwargs) -> None:
    ...
    def forward(self, input):
        ...
        mu_token = torch.tile(self.mu_token, (B,)).reshape(B, 1, -1)   ## 类似于repeat, B个[...(latent_dim=256个满足标准正态分布的随机数)]  （B, 1, latent_dim）
        logvar_token = torch.tile(self.logvar_token, (B,)).reshape(B, 1, -1)   ## 与上述同理

        x = torch.cat([mu_token, logvar_token, x], dim=1)    ## (B, 1+1+seq_len, latent_dim)

        x = x.permute(1, 0, 2)   ## (1+1+seq_len, B, latent_dim)

        token_mask = torch.ones((B, 2), dtype=bool, device=input.get_device())  ## (B, 2),全为True
        mask = lengths_to_mask(lengths, input.get_device())   ## (B, seq_len),全为True
        ...
        logvar = x[1]  # log_var [B, 128]
        std = logvar.exp().pow(0.5)
        dist = torch.distributions.Normal(mu, std)  ##Normal(loc: torch.Size([B, 128]), scale: torch.Size([B, 128]))
        motion_sample = self.sample_from_distribution(dist).to(input.get_device()) # [B, 128]
        return motion_sample, dist

    def sample_from_distribution(self, distribution):
         return distribution.rsample()  ## 从正态分布中生成一个样本, x轴的数值
...
class Decoder(nn.Module):  ## Transformer
    ...
    def forward(self, encoded_feature, past_reaction_3dmm = None, past_reaction_emotion = None):
        B, TS = encoded_feature.shape[0], encoded_feature.shape[1]  ## B, seq_len
        if self.online:
            TL = self.window_size  ## 8
        else:
            TL = TS
...
class SpeakerBehaviourEncoder(nn.Module):
    ...
    def forward(self, video, audio):
        ...
        return  speaker_behaviour_feature  ## B, seq_len, 128
...
class TransformerVAE(nn.Module):
    ...
    def forward(self, speaker_video=None, speaker_audio=None, **kwargs):
        ...
        frame_num = speaker_video.shape[1]  ## seq_len
        distribution = []
        if self.online:
            period = frame_num // self.window_size  ## 751 // 8 == 93
            reaction_3dmm = torch.zeros((speaker_video.size(0), self.window_size, self.output_3dmm_dim), device = speaker_video.get_device()) ## B, 8, 58
            reaction_emotion = torch.zeros((speaker_video.size(0), self.window_size, self.output_emotion_dim), device = speaker_video.get_device())  ## B, 8, 25

            for i in range(0, period):
                speaker_video_, speaker_audio_ = speaker_video[:, : (i + 1) * self.window_size], speaker_audio[:, :  (i + 1) * self.window_size]  ## B, 当前时刻的总帧数, 3, H, W
                encoded_feature = self.speaker_behaviour_encoder(speaker_video_, speaker_audio_)  # B, 当前时刻的总帧数, 128

                # modality fusion
                encoded_feature = self.fusion(torch.cat((encoded_feature, reaction_3dmm, reaction_emotion), dim = -1))  ## B, 8, 128+58+25->128

                if i != 0:
                    past_reaction_3dmm, past_reaction_emotion = reaction_3dmm[:, : i * self.window_size], reaction_emotion[:, : i * self.window_size]  # B, 上一时刻的总帧数, 128
                    current_reaction_3dmm, current_reaction_emotion = reaction_3dmm[:, i * self.window_size :], reaction_emotion[:, i * self.window_size :]  # B, 当前时刻及以后的总帧数, 128
