...
class VAELoss(nn.Module):
    def __init__(self, kl_p=0.0002):
        ...
        self.kl_loss = KLLoss() # KL散度可以用来衡量两个概率分布之间的相似性，两个概率分布越相近，KL散度越小。
        self.kl_p = kl_p
    ...
    def forward(self, gt_emotion, gt_3dmm, pred_emotion, pred_3dmm, distribution):
        ...
        mu_ref = torch.zeros_like(distribution[0].loc).to(gt_emotion.get_device()) # [128]
        scale_ref = torch.ones_like(distribution[0].scale).to(gt_emotion.get_device()) # [128]
        distribution_ref = torch.distributions.Normal(mu_ref, scale_ref) # [128]标准正态分布

        kld_loss = 0
        for t in range(len(distribution)):# B
            kld_loss += self.kl_loss(distribution[t], distribution_ref)
        kld_loss = kld_loss / len(distribution)

        loss = rec_loss + self.kl_p * kld_loss  # kl_p = 0.0002
